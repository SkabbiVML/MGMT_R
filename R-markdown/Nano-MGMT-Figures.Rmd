...
MGMT_RunSum <- read.csv("Data/newRunsum.csv", row.names = 1)
...
barplot_data <- MGMT_RunSum %>% select(c(SampleID, Diagnosis, Known_status)) %>%
  ...
  select(c(Known_status,'IDH-glioma', Glioblastoma, Other)) %>%
...
ggplot(MGMT_RunSum, aes(x=Method, y=Depth))+
  geom_point(size = 5, position = position_jitterdodge(0.2,0), alpha = 0.6, aes(color=Known_status))+
  scale_color_brewer(name="Known Status", palette = "Set1")+
  scale_fill_discrete(guide="none")+
   scale_x_discrete(labels = c("Adaptive", "Barcoded", "Single"))+
  stat_summary(fun=median, geom="point", shape= "-", size=18, color="black",alpha = 0.8,position = position_dodge(0.8), aes(fill=Known_status))+
  #ylim(0,125)+
...
FOUR_CpGs <- read.csv("Data/newFOUR_CPGs.csv", row.names = 1)
...
library(ggpubr)
Compare_CpGs_GROUP <- read.csv("Data/Compare_CpGs_Group.csv", row.names = 1)
