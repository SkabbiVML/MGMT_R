...
X2Ttest <- x2 %>%
  dplyr::select(c(2:4)) %>%
  dplyr::group_by(CpG,Known_status) %>%
  tidyr::nest() %>%
  spread(key = Known_status, value = data) %>%
  dplyr::mutate(
    t_test = map2(Methylated,UnMethylated,~{t.test(.x$percentMeth,.y$percentMeth) %>% tidy()}),
    Methylated = map(Methylated,nrow),
    UnMethylated = map(UnMethylated,nrow)
  ) %>%
  tidyr::unnest(cols = c(Methylated, UnMethylated, t_test))

X2Ttest$adj.p.val <- p.adjust(X2Ttest$p.value, method = "bonferroni", n = length(X2Ttest$p.value))

X2Ttest$CpG <- as.integer(c(-7:-1, 1:108))

ggplot(X2Ttest, aes(x=CpG, y=adj.p.val))+
  geom_point()+
  geom_line()+
  scale_y_log10()+
...
GBMs <- MGMT_RunSum %>% dplyr::select(SampleID,Diagnosis) %>% dplyr::filter(Diagnosis == "Glioblastoma") %>% distinct()
