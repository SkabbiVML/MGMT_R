...
knitr::include_graphics("MGMT_R/R-markdown/Figures/MGMT_promoter_region.png")
```
...
Table <- MGMT_RunSum %>% 
  select(SampleID,Series,Diagnosis) %>% 
  distinct() %>% 
...
barplot_data <- MGMT_RunSum %>% select(c(SampleID, Diagnosis, Known_status)) %>%
  distinct() %>% 
  group_by(Diagnosis, Known_status) %>% 
  tally() %>%
  spread(Diagnosis,n) %>%
  replace(is.na(.), 0) %>%
  mutate(Other = rowSums(select(.,c(LGG_PA,ATRT_SHH, Ganglioglioma, HPC, Medulloblastoma, PCNSL, PXA, Metastasis, Unknown)))) %>%
  mutate('IDH-glioma' = rowSums(select(.,c(Astrocytoma,`Astrocytoma HG`, Oligodendroglioma)))) %>%
  select(c(Known_status,'IDH-glioma',Meningioma, Glioblastoma, Other)) %>%
...
library(ggpubr)
Compare_CpGs_GROUP <- read.csv("R-markdown/Data/Compare_CpGs_Group.csv", row.names = 1)


##### Faceting with DenStem Samples ####################
ggplot(Compare_CpGs_GROUP, aes(x=Pyro_4_CpG_average, y=Nano_4_CpG_average, color = DataSet)) +
  ...
  stat_regline_equation(label.y = 6, label.x = 6,aes(label = ..rr.label..),color = "black", size=4)+
...
ggplot(Compare_CpGs_Radium, aes(x=Pyro_4_CpG_average, y=Nano_4_CpG_average, color = "CornflowerBlue")) +
  geom_smooth(method = "lm", color="black") +
  geom_point(aes(size=Coverage), alpha = 0.6)+
...
=======
FOUR_CpGs <- read.csv("R-markdown/Data/newFOUR_CPGs.csv")
>>>>>>> Stashed changes
...
Depth <- MGMT_RunSum %>% select(c(SampleID,Depth))
#names(coverage) <- c("Series", "Known_status", "SampleID", "Coverage")

t3 <- left_join(t3, Depth, by="SampleID")
...
res <- pheatmap(Metsum_ROI_long_ISLAND,
         ...
         scale = "none",
         drop_levels = T,
         #color = c("blue",brewer.pal(n = 9, name = "Reds")),
...
Patient_meta <- read.csv("R-markdown/Data/Patient_Survival.csv")
