...
```{r Promoter, echo=FALSE, eval=FALSE, out.width="100%", fig.cap="Organization of the MGMT promoter. MSP refers to the typical primer sites of methylation specific PCR to determine MGMT promoter methylation. Asterixes represent the 4 CpGs analysed by the Qiagen MGMT pyrosequencing kit."}
setwd("/home/vml/Documents")
knitr::include_graphics("GitHub/MGMT_R/R-markdown/Figures/MGMT_promoter_region.png")
```
...
ggplot(barplot_data, aes(x=Diagnosis, y= Samples, fill=Known_status, label = Samples))+
  ...
  scale_x_discrete(limits = c("Other", "IDH glioma","Glioblastoma"))+
...
ggplot(MGMT_RunSum, aes(x=Method, y=MED_cov))+
  ...
  scale_fill_brewer(name="Known status", palette="Set1")+
  scale_x_discrete(
    labels = c("WGSeq","Adaptive\nSampling", "Barcoded\nnCATs","Single\nnCATs" ), limits=c("WG_Seq", "AdaptiveSampling", "Barcoded", "Single" )
    )+
...
ggplot(FOUR_CpGs_Group, aes(x=Known_status, y=Average_Nano, color = Known_status))+
  ...
  ylab("Average methylation % nanopore\nCpGs 76-79")+
  theme_bw()+
...
```{r Concordance, echo=FALSE, eval=FALSE}
knitr::kable(p2)
```
...
colnames(STP27) <- c("SampleID", "CpG_31", "CpG_84", "Series", "Method", "Known_status", "Diagnosis" )
...
ggplot(STP27, aes(y=CpG_31, x=CpG_84, color = Known_status)) +
 ...
 # stat_ellipse(level = 0.50, na.rm=T, linetype = 2)+
  ...
  scale_color_brewer(name="Known Status", palette = "Set1")+
  # scale_x_continuous(limits = c(0,100),breaks=c(1,10,25,50,75,100))+
   # scale_y_continuous(limits = c(0,100),breaks=c(1,10,25,50,75,100))+
  guides(colour = guide_legend(override.aes = list(size=5)))+
  theme_bw(base_size = 10)+
  facet_wrap(.~fct_rev(Series))+
  theme(aspect.ratio = 1)
...
MGMT_long_ext_island <- All_MGMT_island %>% 
...
t3 <- t3 %>% filter(Series != "DenStem" & Series != "WGS")
...
##################################################
setwd("/home/vml/Documents")
Predictions <- read.csv2("GitHub/MGMT_R/Data/Areeba_predictions.csv", header = T)

Rapid.Predictions <- MGMT_RunSum %>% 
                      dplyr::select(SampleID,Series, AVG_cov, Known_status) %>% 
...
ggplot(Rapid.Predictions, aes(x=Known_status, y=Model_average, color=Known_status))+
  geom_violin(alpha=0.6)+
  geom_beeswarm(aes(size = AVG_cov), alpha = 0.6, cex=3)+
  scale_size_continuous(name="AVG_cov",breaks = c(5,10,50,100), range = c(1,10) )+
  scale_color_brewer(palette = "Set1", guide="none")+
    #ylim(0,25)+
  xlab("Known methylation status")+
  ylab("Average methylation per nanopore")+
...
x3$CpG_row <- as.integer(rep(c(1:98),2))



ggplot(x3, aes(x=CpG_row, y=Average_Methylation, group =  Known_status))+
